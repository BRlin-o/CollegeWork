/********** EEPROM3.C *****"********************************
*動作：讀取8-bit陣列資料寫入EEPROM 93C66，再重覆讀取到LED輸出
...
main()
{
	...
	for(i=0;i<8;i++)    //寫入8筆資料
   	  WRITE_8(i,Table[i]); //寫入位址及資料
    EWDS();    //禁能EEPROM寫入及清除
    while(1)   //不斷重覆讀取EEPROM的資料
    {  
      for(i=0;i<8;i++) //讀取8筆資料
      {
        LED=READ_8(i); //讀取EEPROM的8-bit資料低位元組輸出
	    Delay_ms(100); //延時
      }
   }
}
...
/********************************************
 函數名稱:WRITE_8
 功能描述:93C66串列EEPROM送入位址及8-bit資料
 輸入參數：Addr,ch
*********************************************/
